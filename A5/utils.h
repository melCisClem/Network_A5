...
GLuint g_fontPlayerName;
stbtt_bakedchar g_dataPlayerName[96];
...
void drawScoreboard(int winW, int winH)
{
    ...
    std::vector<std::pair<uint32_t, ClientPlayer>> sortedPlayers;
    {
        std::lock_guard<std::mutex> lock(g_stateMtx);
        for (const auto& pair : g_renderPlayers)
            sortedPlayers.push_back(pair);
    }
    // sort by kills
    std::sort(sortedPlayers.begin(), sortedPlayers.end(),
        [](const std::pair<uint32_t, ClientPlayer>& a, const std::pair<uint32_t, ClientPlayer>& b) {
    ...
    for (const auto& pair : sortedPlayers)
    {
        ...
        std::string playerStr = pair.second.name;
